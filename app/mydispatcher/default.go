...
// ecycloudNodeManagedPrefixes defines all protocol prefixes that ECYCloudNode manages.
// Tags with these prefixes follow the format: {Protocol}_{IP}_{Port}_{NodeID}
var ecycloudNodeManagedPrefixes = []string{
	"VLESS_",
	"Trojan_",
	"Vmess_",
	"Shadowsocks_",
...
// isECYCloudNodeManagedTag checks if a tag is managed by ECYCloudNode (i.e., it belongs to a specific node).
// ECYCloudNode-managed tags have the format: {Protocol}_{IP}_{Port}_{NodeID}
func isECYCloudNodeManagedTag(tag string) bool {
	for _, prefix := range ecycloudNodeManagedPrefixes {
...
// and adds ECYCloudNode-specific features like rate limiting and rule management.
...
func (d *DefaultDispatcher) DispatchLink(ctx context.Context, destination net.Destination, outbound *transport.Link) error {
	...
	outbound = dispatcher.WrapLink(ctx, d.policy, d.stats, outbound)
	sniffingRequest := content.SniffingRequest
	if !sniffingRequest.Enabled {
		d.routedDispatch(ctx, outbound, destination)
	} else {
...
func (d *DefaultDispatcher) routedDispatch(ctx context.Context, link *transport.Link, destination net.Destination) {
	...
	// 对所有 ECYCloudNode 管理的节点强制执行“源进源出”：入站 tag 必须和出站 tag 完全一致，
	// 否则直接拒绝，防止流量串到其他节点或协议上。
	// ECYCloudNode 节点的 tag 格式为: {Protocol}_{IP}_{Port}_{NodeID}
	...
	isECYCloudNodeNode := isECYCloudNodeManagedTag(inTag)

	if inTag != "" && isECYCloudNodeNode {
		if h := d.ohm.GetHandler(inTag); h != nil {
			handler = h
			isPickRoute = 3 // Matched by inTag (ECYCloudNode same-node)
			errors.LogInfo(ctx, "ECYCloudNode same-node routing: inTag=", inTag, " outboundTag=", h.Tag())
		} else {
			// 对 ECYCloudNode 节点，找不到同名出站时直接拒绝，避免跨节点泄露。
			errors.LogError(ctx, "ECYCloudNode: FATAL - no outbound found for inTag: ", inTag, ", rejecting connection to prevent cross-node routing")
			common.Close(link.Writer)
			common.Interrupt(link.Reader)
			return
		}
	} else {
		// 非 ECYCloudNode 节点或 inTag 为空，走原有路由逻辑。
	...
	if accessMessage := log.AccessMessageFromContext(ctx); accessMessage != nil {
		if tag := handler.Tag(); tag != "" {
			if inTag == "" {
				...
				accessMessage.Detour = inTag + " => " + tag // Matched by inTag (ECYCloudNode node-specific outbound)
			} else {
				accessMessage.Detour = inTag + " >> " + tag
			}
		}
